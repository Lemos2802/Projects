...
%format (lcbr6 (x)(y)(z)(a)(b)(c)) = "\begin{lcbr}" x "\\" y "\\" z "\\" a "\\" b "\\" c "\end{lcbr}"
...
\\
a96484 & Gonçalo Fernandes Lemos
\\
a96115 & Simão Pedro Cunha Matos
\end{tabular}
\end{center}
...
\begin{code}
loop a b c ((f,k),j) = ((c*j + b*k + a*f, f), k)
initial = ((1,1),0)
wrap = p2
\end{code}

Deduzido a partir do conjunto de equações:
...
\xymatrix@@C=2cm{
    |Nat0|
           \ar[d]_-{|cataNat (split (split f k) j)|}
&
    |1 + Nat0|
           \ar[d]^{|id + (cataNat (split (split f k) j))|}
           \ar[l]_-{|inNat|}
\\
     |Nat0|
&
     |1 + (Nat0 * Nat0) * Nat0|
           \ar[l]^-{|[const (((1,1),0)), loop]|}
}
\end{eqnarray*}

Tempo necessário para gerar os 30 primeiros elementos da sequência para f 1 2 3
(|map (f 1 2 3) [0..29]|): 144.59 segundos
Substituindo a função f por fbl: 0.02 segundos
...
gene (x:xs) | a == 1 = i1 x
            | otherwise = i2 (x,b)
    where b = divide xs
          a = length (head (divide (x:xs)))
          divide = groupBy (\x y -> countSpaces x < countSpaces y)
          countSpaces = length . takeWhile (== ' ')
\end{code}
Função de pós-processamento:
\begin{code}
post et = nub (g l1 l2)
  where l1 = map fst $ zip (nub (map fst (tar et))) (map snd (tar et))
        l2 = map (\(a,b) -> a ++ [b]) (tar et)

g :: Eq a => [[a]] -> [[a]] -> [[a]]
g l [] = l
g [] l = l
g (a:b) (c:d) | isPrefixOf a c = (tail (inits a)) ++ [c] ++ g b d
              | otherwise = [c] ++ g (a:b) d
\end{code}

\subsection*{Problema 3}
A partir das coordenadas e largura inicial este gene, utiliza a função init square para
definir o quadrado central e a função arround para definir os quadradros do próximo nível.
\begin{code}

squares = anaRose gsq

gsq = split initSquare arround 
    where arround (((x,y),l),n) =  [(((a,b), l/3), n-1) | b <- [y, y + l/3, y + 2*(l/3)] , 
                                     a <- [x + 2*(l/3), x + l/3, x], (a,b) /= (x + l/3, y + l/3), n>0 ]
...
\subsubsection*{Versão probabilística}
\begin{code}
pinitKnockoutStage = D . (map (\(a,b) -> (initKnockoutStage a, b))) . unD 
